...
BASE_DIR = 'anomalies/amplitude-larger'
FILE_TRAIN = f'train_no_anomaly.csv'
FILE_TEST = f'test.csv'


def make_dataset(config, data):
    return Anomalies(config, data)
...
class Anomalies(Dataset):
    ...
    def get_train_val_data(self):
        ...
        return Anomalies(self.config, data=train_data), Anomalies(self.config, data=val_data)

    def get_test_data(self):
        test_data = pd.read_csv(f'{self.config["data"]["dir"]}/{FILE_TEST}')
        return Anomalies(self.config, data=test_data)
